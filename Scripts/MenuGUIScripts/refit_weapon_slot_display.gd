...
func _on_input_event(event):
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_RIGHT:
		root.player_fleet.add_item(weapon_slot.weapon)
		weapon_slot.weapon = data.weapon_dictionary.get(data.weapon_enum.EMPTY)
		root.update_weapon_list()
	
func _on_focus_entered() -> void:
	weapon_slot.toggle_display_aim(true)
...
func _on_focus_exited() -> void:
	...
	weapon_slot.toggle_display_aim(false)
