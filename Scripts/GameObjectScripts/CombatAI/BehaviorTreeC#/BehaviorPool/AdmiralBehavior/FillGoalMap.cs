...
public partial class FillGoalMap : Action
{
    ...
    {
        if (Engine.GetPhysicsFrames() % 480 != 0) return NodeState.FAILURE;
        ...
        {
            float value = goal_value[cell];
            if (value > 0.0f) norm_val += 1.0f;
        }

        goal_map.ClearMap();
        if (admiral.IsolatedCells.Count == 0) return NodeState.FAILURE;
