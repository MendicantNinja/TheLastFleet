...
func tick(agent: Ship, blackboard: Blackboard) -> int:
	...
	if agent.target_in_range == true:
		...
		for neighbor in friendly_neighbors:
			...
			if square_dist > (agent_fof + neighbor_fof) or square_dist < abs(neighbor_fof - agent_fof):
				continue
			...
			if fof_arc1 <= norm_arc and norm_arc <= fof_arc2:
	...
	velocity = sum_direction * agent.ship_stats.acceleration * agent.time
