...
func tick(agent: Admiral, blackboard: Blackboard) -> int:
	...
	var enemy_vulnerability: Dictionary = agent.enemy_vulnerability
	var chunk_values: Dictionary = {}
	var target_clusters: Array = []
	for cell in enemy_clusters:
		...
		for v_cell in enemy_vulnerability.keys():
			if v_cell.x >= start_idx.x && v_cell.y >= start_idx.y && v_cell.x <= end_idx.x && v_cell.y <= end_idx.y:
				valid_cell = true
				chunk_array.append(enemy_vulnerability[v_cell])
